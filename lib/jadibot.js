...
const jadibot = async(reply,client,id) => {
    ...
    conn.browserDescription = [ 'jadibot', 'Chrome', '3.0' ]
    conn.on('qr', async qr => {
        ...
        fs.existsSync("./jadibot.json") && ikyads.loadAuthInfo("./jadibot.json");
    	setTimeout(() => {
    ...
    conn.on('open', () => {
        ...
        fs.writeFileSync('./jadibot.json', JSON.stringify(conn.base64EncodedAuthInfo(), null, '\t'))

    })
    await conn.connect({timeoutMs: 30 * 1000})
    listjadibot.push(conn.user)
    conn.on('chat-update', async (message) => {
        require('../ziyy.js')(conn, message)
    })
}
